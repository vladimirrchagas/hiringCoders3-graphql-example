...
export const typeDefs = gql`
    ...
    extend type Query {
        client(id: ID): Client
        clients(options: ClientListOption): ClientList
    }
...
export const resolvers = {
    ...
    Mutation: {
        ...
        deleteClient: async (_, { id }) => {
            ...
            if (!currentClient) {
                throw new Error(`Cannot delete client with this id "${id}"`)
            }
        ...
        enableClient: async (_, { id }) => {
            ...
            if (!currentClient.disabled) {
                throw new Error(`Client whose id is "${id}" is already enabled.`)
            }
        ...
        disableClient: async (_, { id }) => {
            ...
            if (currentClient.disabled) {
                throw new Error(`Client whose id is "${id}" is already disabled.`)
            }
